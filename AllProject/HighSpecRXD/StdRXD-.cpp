...
int CHighSpecRXD::PreProcessData()
{
	...
	/*CAlgProcessTime::Alg_start();
	RMatrix tempM(m_iBandCount, iImageSize);
	m_pSrcDS->RasterIO(GF_Read, 0, 0, m_imgWidth, m_imgHeight, &tempM(0,0), 
		m_imgWidth, m_imgHeight, GDT_Float64, m_iBandCount, panBandMap, 0, 0, 0);
	CAlgProcessTime::Alg_end();
	printf("读取数据(矩阵)时间：%lf s\n", CAlgProcessTime::GetAlgProcessTime());*/

	
	
	/*RMatrix zeroMeanMat(m_iBandCount, iImageSize);
	zeroMeanMat = tempM;*/



	//求取列向量均值
	//Eigen::MatrixXd meanVec = tempM.colwise().mean();
	...
	//CAlgProcessTime::Alg_start();
	//tempM.colwise() -= meanVector;
	////计算协方差
	//m_InvCovarianceMatrix = ((tempM*tempM.transpose())/(iImageSize)).inverse();
	//CAlgProcessTime::Alg_end();
	//printf("计算协方差时间：%lf s\n", CAlgProcessTime::GetAlgProcessTime());
...
int CHighSpecRXD::CalcInvCovarianceMartix()
{
	...
	//	m_rowVector.push_back(rowVector);
	...
	for (int i=0; i<m_imgHeight; ++i)
	{
		for (int j=0; j<m_imgWidth; ++j)
		{
			for (int b=0; b<m_iBandCount; ++b)
			{
				...
				rowVector[b] = pBuff1[i*m_imgWidth + j] - m_MeanVector[b];
				colVector[b] = pBuff1[i*m_imgWidth + j] - m_MeanVector[b];
			}
			m_colVector.push_back(colVector);
			m_rowVector.push_back(rowVector);
...
int CHighSpecRXD::ExecuteStdRXD(const char* pszRXDFile, int iBandCount /* = -1 */, const char* pszFormat /* = "GTiff" */)
{
	...
	for (int i=0; i<iImageSize; ++i)
	{
		...
		pOutData[i] = m_rowVector[i]*m_InvCovarianceMatrix*m_colVector[i];
 	}
	CAlgProcessTime::Alg_end();
	printf("RXD计算时间：%lf s\n", CAlgProcessTime::GetAlgProcessTime());
