...
int CHighSpecRXD::ExecuteMatrixRXD(const char* pszRXDFile, int iBandCount /* = -1 */, const char* pszFormat /* = "GTiff" */)
{
	GDALAllRegister();
	CPLSetConfigOption("GDAL_FILENAME_IS_UTF8", "NO");
	...
	RMatrix srcDataMatrix(m_iBandCount, iImageSize);// 按行存储

	// 读取数据至矩阵
	CAlgProcessTime::Alg_start();
	m_pSrcDS->RasterIO(GF_Read, 0, 0, m_imgWidth, m_imgHeight, &srcDataMatrix(0,0), 
	...
	CAlgProcessTime::Alg_start();
	// 计算零均值矩阵
	srcDataMatrix.colwise() -= meanVector;

	// 计算协方差
	m_srcInvCovarianceMatrix = srcDataMatrix*srcDataMatrix.transpose() / iImageSize;
	CAlgProcessTime::Alg_end();
	printf("计算协方差逆矩阵时间：%lf s\n", CAlgProcessTime::GetAlgProcessTime());

	double *pOutData = new double[iImageSize];
	MyMatrix resRXD(1,1);
	RVector rVector(m_iBandCount);
	CVector cVector(m_iBandCount);

	//for (int i=0; i<iImageSize; ++i)
	//{
	//	for (int j=0; j<m_iBandCount; ++j)
	//	{
	//		rVector = srcDataMatrix.row(i);

	//	}
	//	
	//	
	//	//(srcDataMatrix.rowwise())*m_srcInvCovarianceMatrix*(srcDataMatrix.rowwise().transpose());
	//	resRXD = rVector*m_srcInvCovarianceMatrix*rVector.transpose();// 按公式计算
	//	pOutData[i] = resRXD(0, 0);
	//}
