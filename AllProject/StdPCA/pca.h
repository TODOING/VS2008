...
public:
	...
	* @return 返回代码
	*/
	int ExecutePCA(const char* pca_file, const char *statistics_file, int pca_band_count = -1, GDALDataType dst_type = GDT_Float32, bool is_covariance = true, 
	...
	* @return 返回代码
	*/
	int ExecuteInversePCA(const char* inverse_pca_file, const char *statistics_file,  GDALDataType dst_type = GDT_Float32, const char* format = "GTiff");
...
private:
	...
	*/
	...
	int RunPCA(const char *pca_file, int pca_band_count, bool is_float, bool is_like_envi, const char* format);

	template <typename T1, typename T2>
	int RunPCA_New(const char *pca_file, int pca_band_count, bool is_float, bool is_like_envi, const char* format);

	template <typename T1, typename T2>
	int RunInversePCA(const char *inverse_pca_file, MyMatrix &inverse_egeinvectors, bool is_float, const char* format);
