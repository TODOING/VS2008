...
{
	public:
	SSDC(const char *src_file);
	~SSDC();

	/**
	* @brief PC变换
	* @param pszssdcFile			输出主成分变换后文件的路径
	...
	* @return 返回代码
	*/
	int ExecuteSSDC(const char* ssdc_file, const char *statistics_file, int ssdc_band_count = -1, GDALDataType dst_type = GDT_Float32, bool is_covariance = true, 
	...
	* @return 返回代码
	*/
	int ExecuteInverseSSDC(const char* inverse_ssdc_file, const char *statistics_file,  GDALDataType dst_type = GDT_Float32, const char* format = "GTiff");
...
private:
	...
	*/
	...
	int RunSSDC(const char *ssdc_file, int ssdc_band_count, bool is_float, bool is_like_envi, const char* format);

	template <typename T1, typename T2>
	int RunSSDC_New(const char *ssdc_file, int ssdc_band_count, bool is_float, bool is_like_envi, const char* format);

	template <typename T1, typename T2>
	int RunInverseSSDC(const char *inverse_ssdc_file, MyMatrix &inverse_egeinvectors, bool is_float, const char* format);
...
private:	
	...
	bool m_is_covariance;		/*<! ssdc变换方式，true为协方差，false为相关系数 */
